[EXEC_SUMMARY_governance.md](https://github.com/user-attachments/files/22052444/EXEC_SUMMARY_governance.md)
...
- **Module:** g-oss-governance (Lineage & run observability)
- **Purpose:** Practical, open lineage and run history via OpenLineage/Marquez.
- **Primary components:** Marquez (OpenLineage server), Postgres (metadata store)
- **Replaces / reduces:** Lineage add-ons in Collibra/Alation or Informatica EDC lineage
- **Integrates with:** g-oss-batch (openlineage-airflow), dbt, Trino, g-oss-core, g-oss-observability
...
This repository delivers the **g-oss-governance (Lineage & run observability)** segment of that picture.

---

## 4) Common oil & gas use cases (examples)

- RCA for failed runs (trace upstream tables/jobs)
- Regulatory reporting: show provenance and timings
...
- **Lineage coverage > 80%** of scheduled jobs in first phase.
- Every failure links to upstream datasets for rapid RCA.
...
1. **Pilot (0–90 days)**  
   - Stand up `g-oss-governance (Lineage & run observability)` in a sandbox; connect a *low-risk* data domain.  
...
- **Emitter gaps** → Ensure openlineage emitters enabled in Airflow/dbt.
- **Siloed tools** → Standardize lineage IDs across modules.
- **Data sprawl** → Retention policies; archive old runs.
...
- Collibra/Alation lineage add-ons → OpenLineage/Marquez
- Informatica EDC lineage → OpenLineage
